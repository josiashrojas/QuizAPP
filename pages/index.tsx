    ...
    <>
      {
        ...
        <div className="max-w-screen-lg mx-auto mt-10">
          ...
          <h2 className="text-center text-5xl my-10">
            {questions.length === 0 ? "loading..." : decode(questions[currentQuestion].question)}
          </h2>
          <div className="grid grid-cols-2 grid-rows-2 gap-5">
            <Card onClick={handleAnswerClick} className="bg-red-100 hover:bg-red-300" answer={questions.length === 0 ? "loading..." : decode(questions[currentQuestion].allAnswers[0])}/>
            <Card onClick={handleAnswerClick} className="bg-blue-100 hover:bg-blue-300"answer={questions.length === 0 ? "loading..." : decode(questions[currentQuestion].allAnswers[1])}/>
            <Card onClick={handleAnswerClick} className="bg-yellow-100 hover:bg-yellow-300" answer={questions.length === 0 ? "loading..." : decode(questions[currentQuestion].allAnswers[2])}/>
            <Card onClick={handleAnswerClick} className="bg-green-100 hover:bg-green-300" answer={questions.length === 0 ? "loading..." : decode(questions[currentQuestion].allAnswers[3])}/>
          </div>
        </div>
        :
